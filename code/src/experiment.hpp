...
template <typename IN_T, typename OUT_T, template <typename> typename DIST_T>
void continuous_exp(std::string exp_name, DIST_T<IN_T> dist, std::function<OUT_T(std::vector<IN_T> &, size_t)> func, const IN_T input_range_from, const IN_T input_range_to, const size_t numSamples, const size_t numIterations, const size_t numTargets, const size_t numAttackers, const size_t maxNumSpecs, const IN_T step_size) {
    const std::uint64_t seed = 12345;

    std::map<size_t, std::map<IN_T, entType>> awae_data;
    uint k = 1; // 1st NN

    knn_est<IN_T, OUT_T, DIST_T> estimator(seed, dist, func, numSamples, 0, numTargets,k) ;
    for (size_t numSpecs = 1; numSpecs <= maxNumSpecs; numSpecs++) {
